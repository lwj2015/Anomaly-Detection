...
def parse_args():
    ...
    parser.add_argument('--path', nargs='?', default='Data/result4_new_new.xls',
                        help='Input data path.')
    parser.add_argument('--k', type=int, default=10,
                        help='Number of sub_generator.')
    parser.add_argument('--stop_epochs', type=int, default=2000,
...
# Load data
def load_data():
    data = pd.read_excel('{path}'.format(path = args.path),header=None)
...
def plot(train_history):
    ...
    #plt.plot(x, auc_y, color='yellow', linewidth='3',label = 'ROC AUC')
    for i in range(k):
        plt.plot(x, names['gy_' + str(i)], color='green', linewidth='0.5',label = 'Sub-Generator Loss')
...
if __name__ == '__main__':
    ...
    if train:
        ...
        for epoch in range(epochs):
            ...
            # 结果越接近于1代表越接近inlier，越接近于0越接近outlier
            sum = 0.0
    ...
    plot(train_history)
    #np.save('train_history_of_zhusuji', np.array(dict(train_history)))
    #print(data_x)
    #print(data_y)
    #print(p_value)
    #print(np.concatenate((p_value,data_y), axis=1))

    discriminator.save('MO_GAAL_Discriminator_of_zhusuji.h5')